...
DEADLYLOGO = "https://telegra.ph/file/cfdc5cda4f77e6094f9e7.jpg"
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**Deadly"):
            ...
            if IALIVE_PIC:
                DEADLY = [x for x in IALIVE_PIC.split()]
                PIC = list(DEADLY)
                I_IMG = random.choice(PIC)
            if not IALIVE_PIC and ALIVE_PIC:
                DEADLY = [x for x in ALIVE_PIC.split()]
                PIC = list(DEADLY)
                I_IMG = random.choice(PIC)
            elif not IALIVE_PIC:
                I_IMG = None
            ...
            result = builder.article(
                title="© Deadly Help",
                description="Help menu for Deadly",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
            ...
            if PM_PIC:
                DEADLY = [x for x in PM_PIC.split()]
                PIC = list(DEADLY)
                DEADLYIMG = random.choice(PIC)
            else:
                DEADLYIMG = None
            query = gvarstatus("pmpermit_text")
            if DEADLYIMG and DEADLYIMG.endswith((".jpg", ".jpeg", ".png")):
                result = builder.photo(
                    DEADLYIMG,
                    # title="Alive deadly",
                    text=query,
        ...
        buttons = [
            (
                Button.url("Source code", "https://github.com/deadly-fighters/Deadly-Userbot"),
                Button.url(
                    "Deploy",
        ...
        markup = event.client.build_reply_markup(buttons)
        photo = types.InputWebDocument(
            url=DEADLYLOGO, size=0, mime_type="image/jpeg", attributes=[]
        )
        text, msg_entities = await event.client._parse_message_text(
            "𝐃𝐞𝐩𝐥𝐨𝐲 𝐘𝐨𝐮𝐫 𝐎𝐰𝐧 𝐃𝐞𝐚𝐝𝐥𝐲 .", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            ...
            url="https://github.com/deadly-fighters/Deadly-Userbot",
            thumb=photo,
            content=photo,
            send_message=types.InputBotInlineMessageMediaAuto(
                reply_markup=markup, message=text, entities=msg_entities
...
@deadlyub.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@deadlyub.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@deadlyub.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@deadlyub.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
