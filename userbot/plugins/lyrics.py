...
async def lyrics(event):  # sourcery no-metrics
    ...
    if "-" in query:
        ...
        try:
            ...
            songs = None
        if songs is None:
            return await deadlyevent.edit(f"Song **{artist} - {song}** not found!")
        result = f"**Search query**: \n`{artist} - {song}`\n\n```{songs.lyrics}```"
    else:
        deadlyevent = await edit_or_reply(event, f"`Searching lyrics for {query}...`")
    ...
    await edit_or_reply(deadlyevent, result)
