# by @deadly-fighters (@deadly-fighters)
import io
import os
...
async def imirror(event):  # sourcery no-metrics
    ...
    deadlyevent = await event.edit("__Reflecting the image....__")
    args = event.pattern_match.group(1)
    if args:
        filename = "Deadly.webp"
        f_format = "webp"
    else:
        filename = "Deadly.jpg"
        f_format = "jpeg"
...
async def irotate(event):
    ...
    args = event.pattern_match.group(1)
    deadlyevent = await edit_or_reply(event, "__Rotating the replied media...__")
    imag = await _deadlytools.media_to_pic(deadlyevent, reply, noedits=True)
    if imag[1] is None:
        return await edit_delete(
            imag[0], "__Unable to extract image from the replied message.__"
...
async def iresize(event):
    ...
    deadlyevent = await edit_or_reply(event, "__Resizeing the replied media...__")
    imag = await _deadlytools.media_to_pic(deadlyevent, reply, noedits=True)
    if imag[1] is None:
        return await edit_delete(
            imag[0], "__Unable to extract image from the replied message.__"
    ...
    if len(args) == 1:
        try:
            ...
            return await edit_delete(deadlyevent, "**Error:**\n__Invalid dimension.__")
    else:
        try:
            nw = int(args[0])
        except ValueError:
    ...
    try:
        ...
        return await edit_delete(deadlyevent, f"**Error:** __While resizing.\n{e}__")
    await event.delete()
    img = io.BytesIO()
    img.name = "Deadly.png"
    image.save(img, "PNG")
...
async def square_cmd(event):
    ...
    deadlyevent = await event.edit("__Adding borders to make it square....__")
    try:
        imag = await _deadlytools.media_to_pic(deadlyevent, reply, noedits=True)
        if imag[1] is None:
            return await edit_delete(
...
async def pic_gifcmd(event):
    ...
    if args:
        ...
        pix = 100
    deadlyevent = await edit_or_reply(event, "__🎞Dotifying image...__")
    imag = await _deadlytools.media_to_pic(deadlyevent, reply, noedits=True)
    if imag[1] is None:
        return await edit_delete(
            imag[0], "__Unable to extract image from the replied message.__"
