# Heroku manager for your Deadly
...
async def variable(var):  # sourcery no-metrics
    ...
    if exe == "get":
        deadly = await edit_or_reply(var, "`Getting information...`")
        ...
        deadly = await edit_or_reply(var, "`Setting information...`")
        if not variable:
            return await deadly.edit("`.set var <ConfigVars-name> <value>`")
        ...
        if variable in heroku_var:
            await deadly.edit(f"`{variable}` **successfully changed to  ->  **`{value}`")
        else:
            await deadly.edit(
        ...
        deadly = await edit_or_reply(var, "`Getting information to deleting variable...`")
        try:
            variable = var.pattern_match.group(2).split()[0]
        except IndexError:
            return await deadly.edit("`Please specify ConfigVars you want to delete`")
        await asyncio.sleep(1.5)
        if variable not in heroku_var:
            return await deadly.edit(f"`{variable}`**  does not exist**")

        await deadly.edit(f"`{variable}`  **successfully deleted**")
