...
async def cpu(event):
    ...
    cmd = "deadly /proc/cpuinfo | grep 'model name'"
    o = (await _deadlyutils.runcmd(cmd))[0]
    await edit_or_reply(
...
async def sysdetails(sysd):
    ...
    deadlyevent = await edit_or_reply(sysd, "`Fetching system information.`")
    cmd = "git clone https://github.com/dylanaraps/neofetch.git"
    await _deadlyutils.runcmd(cmd)
    neo = "neofetch/neofetch --off --color_blocks off --bold off --cpu_temp C \
                    --cpu_speed on --cpu_cores physical --kernel_shorthand off --stdout"
    a, b, c, d = await _deadlyutils.runcmd(neo)
    result = str(a) + str(b)
    await edit_or_reply(deadlyevent, "**Neofetch Result:** `" + result + "`")
