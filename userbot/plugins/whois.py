...
async def _(event):
    ...
    if not replied_user:
        return
    deadlyevent = await edit_or_reply(event, "`Fetching userinfo wait....`")
...
""".format(
    ...
    await edit_or_reply(deadlyevent, caption)
...
async def who(event):
    ...
    if not replied_user:
        return
    deadly = await edit_or_reply(event, "`Fetching userinfo wait....`")
    replied_user = await event.client(GetFullUserRequest(replied_user.id))
    try:
        photo, caption = await fetch_info(replied_user, event)
    except AttributeError:
        return await edit_or_reply(deadly, "`Could not fetch info of that user.`")
