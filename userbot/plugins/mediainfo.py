...
# ported to telethon by @deadly-fighters (@deadly-fighters)
import os
...
from ..helpers.utils import _format, _deadlyutils
...
async def mediainfo(event):
    ...
    if not reply.media:
        await edit_or_reply(event, "reply to media to get info")
        return
    deadlyevent = await edit_or_reply(event, "`Gathering ...`")
    X_MEDIA = reply.file.mime_type
    if (not X_MEDIA) or (X_MEDIA.startswith(("text"))):
        return await deadlyevent.edit("Reply To a supported Media Format")
    hmm = await file_data(reply)
    file_path = await reply.download_media(Config.TEMP_DIR)
    out, err, ret, pid = await _deadlyutils.runcmd(f"mediainfo '{file_path}'")
    if not out:
        out = "Not Supported"
    body_text = f"""
<h2>JSON</h2>
