...
async def _(event):
    ...
    try:
        ...
        if photo:
            await deadlyevent.edit("`now, Uploading to Telegram ...`")
            if photo.endswith((".mp4", ".MP4")):
                # https://t.me/tgbetachat/324694
                size = os.stat(photo).st_size
                if size > 2097152:
                    await deadlyevent.edit("`size must be less than 2 mb`")
                    os.remove(photo)
                    return
                deadlypic = None
                deadlyvideo = await event.client.upload_file(photo)
            else:
                deadlypic = await event.client.upload_file(photo)
                deadlyvideo = None
            try:
                await event.client(
                    functions.photos.UploadProfilePhotoRequest(
                        file=deadlypic, video=deadlyvideo, video_start_ts=0.01
                    )
                )
            except Exception as e:
                await deadlyevent.edit(f"**Error:**\n`{e}`")
            else:
                await edit_or_reply(
                    deadlyevent, "`My profile picture was successfully changed`"
                )
    try:
        os.remove(photo)
    except Exception as e:
