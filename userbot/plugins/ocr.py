...
from . import _deadlytools, convert_toimage, deEmojify, deadlyub
...
async def ocr(event):
    ...
    deadlyevent = await edit_or_reply(event, "`Reading...`")
    ...
    try:
        ...
        await deadlyevent.edit("`Couldn't read it.. you sure this readable !?`")
    test_file = await ocr_space_file(filename=output_file, language=lang_code)
    try:
        ParsedText = test_file["ParsedResults"][0]["ParsedText"]
    except BaseException:
        await deadlyevent.edit("`Couldn't read it.`\n`I guess I need new glasses.`")
    else:
        if cmd == "":
