...
from ..helpers.utils import _format, _deadlyutils, reply_id
...
async def currency(event):
    ...
    try:
        ...
        symbols = await AioHttp().get_raw(
            "https://raw.githubusercontent.com/deadly-fighters/Deadly-Userbot-Resources/master/Resources/Data/currency.py"
        )
...
async def _(event):
    ...
    chat = "@VS_Robot"
    deadlyevent = await edit_or_reply(event, " `Sliding my tip, of fingers over it`")
    async with event.client.conversation(chat) as conv:
        ...
        if not input_str:
            return await edit_or_reply(deadlyevent, response4.text)
...
async def parseqr(event):
    ...
    deadlyevent = await edit_or_reply(event, "`Decoding....`")
    ...
    t_response, e_response = (await _deadlyutils.runcmd(command_to_exec))[:2]
    if os.path.exists(downloaded_file_name):
        os.remove(downloaded_file_name)
    soup = BeautifulSoup(t_response, "html.parser")
    try:
        qr_contents = soup.find_all("pre")[0].text
        await edit_or_reply(deadlyevent, f"**The decoded message is :**\n`{qr_contents}`")
    except IndexError:
        result = soup.text
        await edit_or_reply(deadlyevent, f"**Failed to Decode:**\n`{result}`")
    except Exception as e:
        await edit_or_reply(deadlyevent, f"**Error:**\n`{e}`")
...
async def _(event):
    ...
    try:
        ...
        return await deadlyevent.edit(str(e))
    end = datetime.now()
    ms = (end - start).seconds
    await edit_delete(deadlyevent, "Created BarCode in {} seconds".format(ms))
...
async def _(event):
    ...
    if not r.ok:
        return await deadlyevent.edit("xkcd n.{} not found!".format(xkcd_id))
    data = r.json()
    year = data.get("year")
