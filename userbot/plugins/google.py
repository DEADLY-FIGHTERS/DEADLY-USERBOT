# reverse search and google search  plugin for deadly
import io
import os
import re
import urllib
...
async def gsearch(q_event):
    ...
    deadlyevent = await edit_or_reply(q_event, "`searching........`")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                return await edit_delete(deadlyevent, f"**Error:**\n`{e}`", time=10)
    msg = ""
    for i in range(lim):
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        deadlyevent = await edit_or_reply(event, "Pre Processing Media")
        ...
        if previous_message.media:
            ...
            the_location = google_rs_response.headers.get("Location")
        await deadlyevent.edit("Found Google Result. Pouring some soup on it!")
        headers = {
        ...
        try:
            ...
            return await edit_delete(
                deadlyevent, "`Sorry. I am unable to find similar images`"
            )
        end = datetime.now()
...
<i>fetched in {ms} seconds</i>""".format(
        ...
        deadlyevent = event
    await edit_or_reply(deadlyevent, OUTPUT_STR, parse_mode="HTML", link_preview=False)
...
async def _(img):
    ...
    if message and message.media:
        ...
        return
    if photo:
        deadlyevent = await edit_or_reply(img, "`Processing...`")
        try:
            image = Image.open(photo)
        except OSError:
            return await deadlyevent.edit("`Unsupported , most likely.`")
        name = "okgoogle.png"
        image.save(name, "PNG")
        image.close()
        ...
        if response != 400:
            ...
            return await deadlyevent.edit("`Unable to perform reverse search.`")
        ...
        if guess and imgspage:
            ...
            return await deadlyevent.edit("`Can't find any kind similar images.`")
