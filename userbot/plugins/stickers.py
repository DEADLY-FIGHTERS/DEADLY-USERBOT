...
from ..helpers.utils import _deadlytools
from ..sql_helper.globals import gvarstatus

plugin_category = "fun"

# modified and developed by @deadly-fighters
...
def verify_cond(deadlyarray, text):
    return any(i in text for i in deadlyarray)


def pack_name(userid, pack, is_anim):
    if is_anim:
        return f"Deadly_{userid}_{pack}_anim"
    return f"Deadly_{userid}_{pack}"
...
async def newpacksticker(
    ...
    try:
        ...
        await deadlyevent.edit("You have blocked the @stickers bot. unblock it and try.")
        if not pkang:
...
async def add_to_pack(
    ...
    while ("50" in x.text) or ("120" in x.text):
        ...
        await deadlyevent.edit(f"`Switching to Pack {pack} due to insufficient space`")
        await conv.send_message(packname)
        x = await conv.get_response()
...
async def kang(args):  # sourcery no-metrics
    ...
    if not user.username:
        try:
            ...
            username = f"deadly_{user.id}"
    else:
        username = user.username
    userid = user.id
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            deadlyevent = await edit_or_reply(args, f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
    ...
    if photo:
        ...
        if len(splat) == 2:
            if char_is_emoji(splat[0][0]):
                if char_is_emoji(splat[1][0]):
                    return await deadlyevent.edit("check `.info stickers`")
        ...
        if (
            ...
            await deadlyevent.edit("`Brewing a new Pack...`")
            async with args.client.conversation("@Stickers") as conv:
...
async def pack_kang(event):  # sourcery no-metrics
    ...
    for message in reqd_sticker_set.documents:
        if "image" in message.mime_type.split("/"):
            ...
            await edit_delete(deadlyevent, "`Unsupported File!`")
            return
        if photo:
            ...
            if (
                ...
                async with event.client.conversation("@Stickers") as conv:
                    pack, Deadlyname = await newpacksticker(
                        deadlyevent,
                        conv,
                        cmd,
                        event,
                ...
                async with event.client.conversation("@Stickers") as conv:
                    pack, Deadlyname = await add_to_pack(
                        deadlyevent,
                        conv,
                        event,
                        packname,
            ...
            if Deadlyname is None:
                return
            if Deadlyname not in blablapacks:
                blablapacks.append(Deadlyname)
...
async def pic2packcmd(event):
    ...
    deadlyevent = await edit_or_reply(event, "__🔪Cropping and adjusting the image...__")
    try:
    ...
    image = await _deadlytools.media_to_pic(deadlyevent, reply, noedits=True)
    if image[1] is None:
        return await edit_delete(
    ...
    newimg.save(new_img)
    new_img.seek(0)
    deadlyevent = await event.edit("__Making the pack.__")
...
async def get_pack_info(event):
    ...
    if not isinstance(stickerset_attr, DocumentAttributeSticker):
        return await deadlyevent.edit("`This is not a sticker. Reply to a sticker.`")
    get_stickerset = await event.client(
        GetStickerSetRequest(
            InputStickerSetID(
                id=stickerset_attr.stickerset.id,
...
async def cb_sticker(event):
    ...
    deadlyevent = await edit_or_reply(event, "`Searching sticker packs....`")
    scraper = cloudscraper.create_scraper()
    text = scraper.get(combot_stickers_url + split).text
    soup = bs(text, "lxml")
    results = soup.find_all("div", {"class": "sticker-pack__header"})
    if not results:
        return await edit_delete(deadlyevent, "`No results found :(.`", 5)
