...
@deadlyub.deadly_cmd(
    ...
    info={
        ...
        "examples": ["{tr}qpic Deadly.", "{tr}qpic -b Deadly."],
    },
)
async def q_pic(event):  # sourcery no-metrics
    ...
    deadlyevent = await edit_or_reply(event, "__Making Quote pic....__")
    ...
    if (
        ...
        imag = await _deadlytools.media_to_pic(event, reply, noedits=True)
        if imag[1] is None:
            return await edit_delete(
    ...
    output = io.BytesIO()
    if sticker:
        output.name = "Deadly.Webp"
        img.save(output, "webp")
    else:
        output.name = "Deadly.png"
        img.save(output, "PNG")
    output.seek(0)
...
async def stickerchat(deadlyquotes):
    "Makes your message as sticker quote"
    reply = await deadlyquotes.get_reply_message()
    if not reply:
        return await edit_or_reply(
            deadlyquotes, "`I cant quote the message . reply to a message`"
        )
    fetchmsg = reply.message
    repliedreply = None
    mediatype = media_type(reply)
    if mediatype and mediatype in ["Photo", "Round Video", "Gif"]:
        return await edit_or_reply(deadlyquotes, "`Replied message is not supported now`")
    deadlyevent = await edit_or_reply(deadlyquotes, "`Making quote...`")
    user = (
        await deadlyquotes.client.get_entity(reply.forward.sender)
        if reply.fwd_from
        else reply.sender
    )
    res, deadlymsg = await process(fetchmsg, user, deadlyquotes.client, reply, repliedreply)
    if not res:
        return
    outfi = os.path.join("./temp", "sticker.png")
    deadlymsg.save(outfi)
    endfi = convert_tosticker(outfi)
    await deadlyquotes.client.send_file(deadlyquotes.chat_id, endfi, reply_to=reply)
    await deadlyevent.delete()
    os.remove(endfi)
...
async def _(event):
    ...
    chat = "@QuotLyBot"
    deadlyevent = await edit_or_reply(event, "```Making a Quote```")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            if messages_id != []:
                ...
                return await edit_delete(
                    deadlyevent, "`I guess you have used a invalid syntax`"
                )
            response = await response
        except YouBlockedUserError:
            return await deadlyevent.edit("```Please unblock me (@QuotLyBot) u Nigga```")
