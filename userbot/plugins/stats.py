...
async def stats(event):  # sourcery no-metrics
    ...
    deadly = await edit_or_reply(event, STAT_INDICATION)
...
async def stats(event):  # sourcery no-metrics
    deadlycmd = event.pattern_match.group(1)
    deadlyevent = await edit_or_reply(event, STAT_INDICATION)
    start_time = time.time()
...
async def _(event):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await edit_delete(deadlyevent, "`unblock `@tgscanrobot` and then try`")
        response = await conv.get_response()
        await event.client.send_read_acknowledge(conv.chat_id)
        await deadlyevent.edit(response.text)
