...
You have {warns}/{totalwarns} warns until you get blocked by the Deadly.
    ...
    try:
        if gvarstatus("pmmenu") is None:
            ...
            if PM_PIC:
                ...
                PIC = list(DEADLY)
                DEADLY_IMG = random.choice(PIC)
            else:
                DEADLY_IMG = None
            if DEADLY_IMG is not None:
...
@deadlyub.deadly_cmd(incoming=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@deadlyub.deadly_cmd(outgoing=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@deadlyub.tgbot.on(CallbackQuery(data=re.compile(rb"show_pmpermit_options")))
...
@deadlyub.tgbot.on(CallbackQuery(data=re.compile(rb"to_enquire_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
...
@deadlyub.tgbot.on(CallbackQuery(data=re.compile(rb"to_request_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "Idoit this options for user who messages you. not for you"
        return await event.answer(text, cache_time=0, alert=True)
...
@deadlyub.tgbot.on(CallbackQuery(data=re.compile(rb"to_chat_with_my_master")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
...
@deadlyub.tgbot.on(CallbackQuery(data=re.compile(rb"to_spam_my_master_inbox")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "Idoit these options are for users who message you. not for you"
        return await event.answer(text, cache_time=0, alert=True)
