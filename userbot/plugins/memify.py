# Made by @deadly-fighters and @deadly-fighters
# memify plugin for Deadly
import asyncio
import base64
import io
import os
...
from ..helpers import asciiart, deadly_meeme, deadly_meme, media_type
...
from ..helpers.utils import _deadlytools, reply_id
...
async def maccmd(event):  # sourcery no-metrics
    ...
    deadlyevent = await event.edit("__Adding frame for media....__")
    args = event.pattern_match.group(1)
    force = bool(args)
    try:
        imag = await _deadlytools.media_to_pic(deadlyevent, reply, noedits=True)
        if imag[1] is None:
            return await edit_delete(
        ...
        return await edit_delete(deadlyevent, f"**Error in identifying image:**\n__{e}__")
    wid, hgt = image.size
...
async def memes(event):
    ...
    deadlyinput = event.pattern_match.group(2)
    reply = await event.get_reply_message()
    if not reply:
        return await edit_delete(event, "`Reply to supported Media...`")
    deadlyid = await reply_id(event)
    ...
    if ";" in deadlyinput:
        top, bottom = deadlyinput.split(";", 1)
    else:
        top = deadlyinput
        bottom = ""
    if not os.path.isdir("./temp"):
        os.mkdir("./temp")
    output = await _deadlytools.media_to_pic(event, reply)
    if output[1] is None:
        return await edit_delete(
    ...
    meme = os.path.join("./temp", "deadlymeme.jpg")
    if gvarstatus("CNG_FONTS") is None:
        CNG_FONTS = "userbot/helpers/styles/Vampires.ttf"
    else:
        CNG_FONTS = gvarstatus("CNG_FONTS")
    if max(len(top), len(bottom)) < 21:
        await deadly_meme(CNG_FONTS, top, bottom, meme_file, meme)
    else:
        await deadly_meeme(top, bottom, CNG_FONTS, meme_file, meme)
    if cmd != "mmf":
        meme = convert_tosticker(meme)
    await event.client.send_file(
        event.chat_id, meme, reply_to=deadlyid, force_document=False
    )
...
async def lang(event):
    ...
    if input_str not in font_list:
        ...
        await deadlyevent.edit(f"**Available Fonts names are here:-**\n\n{FONTS}")
    else:
...
async def memes(event):
    ...
    deadlyinput = event.pattern_match.group(1)
    reply = await event.get_reply_message()
    if not reply:
        return await edit_delete(event, "`Reply to supported Media...`")
    hmm = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    ...
    color1 = c_list[0]
    color2 = c_list[1]
    bgcolor = "#080808" if not deadlyinput else deadlyinput
    asciiart(meme_file, 0.3, 1.9, outputfile, color1, color2, bgcolor)
    await event.client.send_file(
        event.chat_id, outputfile, reply_to=deadlyid, force_document=False
    )
    await output[0].delete()
...
async def memes(event):
    ...
    await event.client.send_file(
        event.chat_id, outputfile, force_document=False, reply_to=deadlyid
    )
    await output[0].delete()
    for files in (outputfile, meme_file):
        if files and os.path.exists(files):
