...
async def _(event):
    ...
    if input_str in ["media", "m"]:
        ...
        await deadlyevent.edit(f"`Downloaded to {downloaded_file_name}`")
        ...
        ms = (end - start).seconds
        deadly = f"https://telegra.ph/{response['path']}"
        await deadlyevent.edit(
            f"**➥ Uploaded to :-** [telegraph]({deadly})\
                 \n**➥ Uploaded in {ms} seconds.**\
                 \n**➥ Uploaded by :-** {mention}",
            link_preview=True,
        )
