...
async def get_curr_track(lfmbio):  # sourcery no-metrics
    ...
    while LASTFM_.LASTFMCHECK:
        try:
            ...
            user_info = await deadlyub(GetFullUserRequest(LASTFM_.USER_ID))
            ...
            if (
                ...
                try:
                    ...
                    await deadlyub(UpdateProfileRequest(about=lfmbio))
                except AboutTooLongError:
                    short_bio = f"🎧: {LASTFM_.SONG}"
                    await deadlyub(UpdateProfileRequest(about=short_bio))
            if playing is None and user_info.about != DEFAULT_BIO:
                await sleep(6)
                await deadlyub(UpdateProfileRequest(about=DEFAULT_BIO))
                if BOTLOG and LASTFM_.LastLog:
                    await deadlyub.send_message(
            ...
            if BOTLOG and LASTFM_.LastLog:
                await deadlyub.send_message(BOTLOG_CHATID, f"Error changing bio:\n{err}")
        await sleep(2)
