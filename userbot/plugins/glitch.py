...
ported to telethon by @deadly-fighters and @deadly-fighters
"""
...
from ..helpers.utils import _deadlytools, _deadlyutils, reply_id
...
async def glitch(event):
    ...
    if not os.path.isdir("./temp"):
        os.mkdir("./temp")
    deadlyinput = int(deadlyinput) if deadlyinput else 2
    glitch_file = await _deadlytools.media_to_pic(event, reply)
    ...
    if cmd:
        ...
        glitch_img = glitcher.glitch_image(img, deadlyinput, color_offset=True)
        glitch_img.save(glitched)
        await event.client.send_file(event.chat_id, glitched, reply_to=deadlyid)
    else:
        glitched = os.path.join("./temp", "glitched.gif")
        glitch_img = glitcher.glitch_image(img, deadlyinput, color_offset=True, gif=True)
        DURATION = 200
        LOOP = 0
        glitch_img[0].save(
            glitched,
        ...
        nadan = await event.client.send_file(event.chat_id, glitched, reply_to=deadlyid)
        await _deadlyutils.unsavegif(event, nadan)
