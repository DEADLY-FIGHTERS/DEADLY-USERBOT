...
async def download_audio(event):
    ...
    deadlyevent = await edit_or_reply(event, "`Preparing to download...`")
    reply_to_id = await reply_id(event)
    try:
        vid_data = YoutubeDL({"no-playlist": True}).extract_info(url, download=False)
    except ExtractorError:
        vid_data = {"title": url, "uploader": "Deadlyub", "formats": []}
    startTime = time()
    ...
    for _path in glob.glob(os.path.join(Config.TEMP_DIR, str(startTime), "*")):
        if _path.lower().endswith((".jpg", ".png", ".webp")):
            ...
            _fpath = _path
    if not _fpath:
        return await edit_delete(deadlyevent, "__Unable to upload file__")
...
async def download_video(event):
    ...
    deadlyevent = await edit_or_reply(event, "`Preparing to download...`")
    reply_to_id = await reply_id(event)
    ytdl_data = await ytdl_down(deadlyevent, video_opts, url)
    if ytdl_down is None:
        return
    f = pathlib.Path(f"{ytdl_data['title']}.mp4".replace("|", "_"))
    deadlythumb = pathlib.Path(f"{ytdl_data['title']}.jpg".replace("|", "_"))
    if not os.path.exists(deadlythumb):
        deadlythumb = pathlib.Path(f"{ytdl_data['title']}.webp".replace("|", "_"))
    if not os.path.exists(deadlythumb):
        deadlythumb = None
    await deadlyevent.edit(
        f"`Preparing to upload video:`\
    ...
    uploaded = await event.client.fast_upload_file(
        file=ul,
        progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
            progress(d, t, deadlyevent, c_time, "upload", file_name=f)
        ),
    )
    ul.close()
    media = types.InputMediaUploadedDocument(
        ...
        mime_type=mime_type,
        attributes=attributes,
        thumb=await event.client.upload_file(deadlythumb) if deadlythumb else None,
    )
    await event.client.send_file(
        event.chat_id,
        file=media,
...
async def kakashi(event):
    ...
    if "www.instagram.com" not in link:
        ...
        start = datetime.now()
        deadlyevent = await edit_or_reply(event, "**Downloading.....**")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await deadlyevent.edit("**Error:** `unblock` @instasavegrambot `and retry!`")
            return
        await deadlyevent.delete()
        deadly = await event.client.send_file(
            event.chat_id,
            video,
        )
        end = datetime.now()
