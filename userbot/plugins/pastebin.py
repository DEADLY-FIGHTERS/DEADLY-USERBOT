...
async def paste_img(event):
    ...
    d_file_name = None
    deadlyevent = await edit_or_reply(event, "`Pasting the text on image`")
    ...
    try:
        ...
        await edit_delete(deadlyevent, f"**Error:**\n`{e}`", time=10)
...
async def paste_bin(event):
    ...
    deadlyevent = await edit_or_reply(event, "`pasting text to paste bin....`")
    ...
    try:
        ...
        await deadlyevent.edit(result, link_preview=False, parse_mode="html")
    except Exception as e:
        await edit_delete(deadlyevent, f"**Error while pasting text:**\n`{e}`")
...
async def get_dogbin_content(event):
    ...
    if not url and textx.text:
        ...
        for iurl in urls:
            if (
                ...
                or ("spaceb" in iurl)
                or ("nekobin" in iurl)
                or ("deadlybin" in iurl)
            ):
                url = iurl
                break
    if not url:
        return await edit_delete(event, "__I can't find any pastebin link.__")
    deadlyevent = await edit_or_reply(event, "`Getting Contents of pastebin.....`")
    rawurl = None
    if "raw" in url:
        rawurl = url
    if rawurl is None:
        ...
        elif "deadlybin" in url:
            rawurl = f"http://deadlybin.up.railway.app/raw/{fid[0]}"
        else:
    ...
    try:
        ...
        return await deadlyevent.edit(f"**Request timed out.**__{str(TimeoutErr)}__")
    ...
    await edit_or_reply(deadlyevent, reply_text)
...
async def _(event):
    ...
    try:
        ...
        return await edit_delete(deadlyevent, f"**Error while pasting text:**\n`{e}`")
    url = response["url"]
    chat = "@CorsaBot"
    await deadlyevent.edit("`Making instant view...`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            return await deadlyevent.edit("```Please unblock me (@CorsaBot) and try```")
        result = ""
        if response:
        ...
        await deadlyevent.edit(result, link_preview=True)
