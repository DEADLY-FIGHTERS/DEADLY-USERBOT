# by  @deadly-fighters ( https://t.me/deadly-fighters  )

# songs finder for Deadly
import asyncio
import base64
import io
import os
...
from ..helpers.utils import _deadlyutils, reply_id
...
async def _(event):
    ...
    deadlyevent = await edit_or_reply(event, "`wi8..! I am finding your song....`")
    ...
    try:
        ...
        pass
    stderr = (await _deadlyutils.runcmd(song_cmd))[1]
    if stderr:
        return await deadlyevent.edit(f"**Error :** `{stderr}`")
    deadlyname, stderr = (await _deadlyutils.runcmd(name_cmd))[:2]
    if stderr:
        return await deadlyevent.edit(f"**Error :** `{stderr}`")
    # stderr = (await runcmd(thumb_cmd))[1]
    deadlyname = os.path.splitext(deadlyname)[0]
    # if stderr:
    #    return await deadlyevent.edit(f"**Error :** `{stderr}`")
    song_file = Path(f"{deadlyname}.mp3")
    if not os.path.exists(song_file):
        return await deadlyevent.edit(
            f"Sorry!. I can't find any related video/audio for `{query}`"
        )
    await deadlyevent.edit("`yeah..! i found something wi8..🥰`")
    deadlythumb = Path(f"{deadlyname}.jpg")
    if not os.path.exists(deadlythumb):
        deadlythumb = Path(f"{deadlyname}.webp")
    elif not os.path.exists(deadlythumb):
        deadlythumb = None
    ...
    for files in (deadlythumb, song_file):
...
async def _(event):
    ...
    stderr = (await _deadlyutils.runcmd(video_cmd))[1]
    if stderr:
        return await deadlyevent.edit(f"**Error :** `{stderr}`")
    deadlyname, stderr = (await _deadlyutils.runcmd(name_cmd))[:2]
    if stderr:
    ...
    #    return await deadlyevent.edit(f"**Error :** `{stderr}`")
    deadlyname = os.path.splitext(deadlyname)[0]
    vsong_file = Path(f"{deadlyname}.mp4")
    if not os.path.exists(vsong_file):
        vsong_file = Path(f"{deadlyname}.mkv")
    ...
    await deadlyevent.delete()
    for files in (deadlythumb, vsong_file):
        if files and os.path.exists(files):
            os.remove(files)
...
async def shazamcmd(event):
    ...
    deadlyevent = await edit_or_reply(event, "__Downloading the audio clip...__")
    try:
        ...
        LOGS.error(e)
        return await edit_delete(
            deadlyevent, f"**Error while reverse searching song:**\n__{e}__"
        )
...
async def _(event):
    ...
    deadlyevent = await edit_or_reply(event, SONG_SEARCH_STRING, parse_mode="html")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            if baka[0].message.startswith(
                ...
                return await edit_delete(
                    deadlyevent, SONG_NOT_FOUND, parse_mode="html", time=5
                )
            await deadlyevent.edit(SONG_SENDING_STRING, parse_mode="html")
            ...
            return await deadlyevent.edit(SONGBOT_BLOCKED_STRING, parse_mode="html")
        await event.client.send_file(
            event.chat_id,
            music,
...
async def _(event):
    ...
    chat = "@auddbot"
    deadlyevent = await edit_or_reply(event, "```Identifying the song```")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await deadlyevent.edit("Wait just a sec...")
            result = await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
        except YouBlockedUserError:
            await deadlyevent.edit("```Please unblock (@auddbot) and try again```")
            return
    namem = f"**Song Name : **`{result.text.splitlines()[0]}`\
        \n\n**Details : **__{result.text.splitlines()[2]}__"
    await deadlyevent.edit(namem)
