...
async def very(event):
    ...
    deadlyevent = await edit_or_reply(event, "`Processing.....`")
    ...
    LOGO_BACKGROUND = (
        ...
        or f"https://raw.githubusercontent.com/deadly-fighters/Files/main/background/black.jpg"
    )
    LOGO_FONT = (
        gvarstatus("LOGO_FONT")
        or f"https://github.com/deadly-fighters/Files/blob/main/fonts/Streamster.ttf?raw=true"
    )
...
async def bad(event):
    ...
    source = requests.get("https://github.com/deadly-fighters/Files/tree/main/backgroud")
    ...
    for i, each in enumerate(links, start=1):
        deadly = os.path.splitext(each.text)[0]
        bg_name.append(deadly)
        lbg_list += f"**{i}.**  `{deadly}`\n"
    ...
    if cmd == "c":
        ...
        if not input_str and event.reply_to_msg_id and reply_message.media:
            if not os.path.isdir("./temp"):
                os.mkdir("./temp")
            output = await _deadlytools.media_to_pic(event, reply_message)
    ...
    if input_str not in bg_name:
        deadlyevent = await edit_or_reply(event, "`Give me a correct background name...`")
        await asyncio.sleep(1)
        await edit_delete(deadlyevent, lbg_list, time=60)
    else:
        string = f"https://raw.githubusercontent.com/deadly-fighters/Files/main/backgroud/{input_str}.jpg"
...
async def pussy(event):
    ...
    input_str = event.pattern_match.group(2)
    if cmd == "":
        source = requests.get("https://github.com/deadly-fighters/Files/tree/main/fonts")
        soup = BeautifulSoup(source.text, features="html.parser")
        links = soup.find_all("a", class_="js-navigation-open Link--primary")
        logo_font = []
        ...
        for i, each in enumerate(links, start=1):
            deadly = os.path.splitext(each.text)[0]
            logo_font.append(deadly)
            font_name += f"**{i}.**  `{deadly}`\n"
        if not input_str:
            return await edit_delete(event, font_name, time=80)
        if input_str not in logo_font:
            deadlyevent = await edit_or_reply(event, "`Give me a correct font name...`")
            await asyncio.sleep(1)
            await edit_delete(deadlyevent, font_name, time=80)
        else:
            if " " in input_str:
                input_str = str(input_str).replace(" ", "%20")
            string = f"https://github.com/deadly-fighters/Files/blob/main/fonts/{input_str}.ttf?raw=true"
        ...
        if input_str not in fg_name:
            deadlyevent = await edit_or_reply(event, "`Give me a correct color name...`")
            await asyncio.sleep(1)
            await edit_delete(
                deadlyevent,
                f"**Available color names are here:-**\n\n{fg_list}",
        ...
        deadly = re.compile(r"^\-?[1-9][0-9]*\.?[0-9]*")
        isint = re.match(deadly, input_str)
