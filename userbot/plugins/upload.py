...
async def deadlylst_of_files(path):
    files = []
...
def sortthings(contents, path):
    ...
    for file in contents:
        deadlypath = os.path.join(path, file)
        if os.path.isfile(deadlypath):
            deadlysort.append(file)
    for file in contents:
        deadlypath = os.path.join(path, file)
        if os.path.isdir(deadlypath):
            deadlysort.append(file)
    return deadlysort
...
async def upload(path, event, udir_event, deadlyflag=None):  # sourcery no-metrics
    deadlyflag = deadlyflag or False
    ...
    if os.path.isdir(path):
        ...
        for file in Files:
            deadlypath = os.path.join(path, file)
            await upload(Path(deadlypath), event, udir_event)
...
async def uploadir(event):
    ...
    if os.path.isdir(path):
        ...
        UPLOAD_.uploaded = 0
        await upload(path, event, udir_event, deadlyflag=flag)
        end = datetime.now()
