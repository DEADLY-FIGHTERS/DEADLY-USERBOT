...
async def grpinfo():
    outstr = "**Plugins in Deadly are:**\n\n"
    outstr += f"**👩‍💻 Usage : ** `{cmdprefix}help <plugin name>`\n\n"
    category = ["admin", "fun", "tools", "utils"]
    for deadly in category:
        plugins = GRP_INFO[deadly]
        outstr += f"**{hemojis[deadly]} {deadly.title()} **({len(plugins)})\n"
...
async def cmdlist():
    outstr = "**Total list of Commands in your Deadly are :**\n\n"
    category = ["admin", "fun", "tools", "utils"]
    for deadly in category:
        plugins = GRP_INFO[deadly]
        outstr += f"**{hemojis[deadly]} {deadly.title()} ** - {len(plugins)}\n\n"
...
@deadlyub.deadly_cmd(
    ...
    info={
        "header": "To get guide for Deadly.",
...
async def _(event):
    ...
    if not input_str:
        ...
        outstr += f"**👩‍💻 Usage : ** `{cmdprefix}help -c <command name>`"
    await edit_or_reply(
        event, outstr, aslink=True, linktext="Total Commands of Deadly are :"
    )
...
async def _(event):
    ...
    if found:
        ...
        out = f"I can't find any such command `{cmd}` in Deadly"
