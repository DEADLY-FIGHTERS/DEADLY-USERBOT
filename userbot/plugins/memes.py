...
# Deadly module for having some fun with people.
import asyncio
import random
import re
...
from ..helpers import deadlymemes
from ..helpers.utils import _deadlyutils, parse_pre
...
@deadlyub.deadly_cmd(
    ...
    info={
        ...
        "examples": [
            "{tr}squirrelsay Deadly",
            "{tr}milksay Deadly",
            "{tr}ghostbustersghostbusterssay Deadly",
        ],
    },
)
...
async def who(event):
    ...
    if replied_user is None:
        return
    caption = await deadlymemes.slap(replied_user, event, mention)
    try:
...
async def faces(owo):
    ...
    reply_text = re.sub(r"\!+", " " + random.choice(deadlymemes.UWUS), reply_text)
    reply_text = reply_text.replace("ove", "uv")
    reply_text += " " + random.choice(deadlymemes.UWUS)
...
async def gbun(event):
    ...
    deadlyevent = await edit_or_reply(event, "**Summoning out le Gungnir ❗️⚜️☠️**")
    ...
    if event.reply_to_msg_id:
        ...
        if idd == 1035034432:
            ...
            if len(gbunVar) > 0:
                ...
                jnl += no_reason
            await deadlyevent.edit(jnl)
    else:
        mention = "`Warning!! User 𝙂𝘽𝘼𝙉𝙉𝙀𝘿 By Admin...\nReason: Potential spammer. `"
        await deadlyevent.edit(mention)
