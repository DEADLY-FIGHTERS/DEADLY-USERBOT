...
async def ban_user(chat_id, i, rights):
    try:
        await deadlyub(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
...
async def _(event):
    ...
    deadlyevent = await edit_or_reply(event, "`Kicking...`")
...
async def rm_deletedacc(show):  # sourcery no-metrics
    ...
    if con != "clean":
        ...
        if flag != " -r":
            ...
            deadlyadmin = await is_admin(show.client, show.chat_id, show.client.uid)
            if not deadlyadmin:
