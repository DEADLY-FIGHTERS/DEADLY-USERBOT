...
from ..helpers.utils import _deadlyutils
...
async def spam_function(event, nadan, deadly, sleeptimem, sleeptimet, DelaySpam=False):
    # sourcery no-metrics
    counter = int(deadly[0])
    if len(deadly) == 2:
        spam_message = str(deadly[1])
...
async def spammer(event):
    ...
    await spam_function(event, nadan, deadly, sleeptimem, sleeptimet)
...
@deadlyub.deadly_cmd(
    ...
    info={
        ...
        "usage": "{tr}wspam <text>",
        "examples": "{tr}wspam I am using Deadly",
    },
)
async def tmeme(event):
...
async def spammer(event):
    ...
    await event.delete()
    addgvar("spamwork", True)
    await spam_function(event, reply, deadly, sleeptimem, sleeptimet, DelaySpam=True)
