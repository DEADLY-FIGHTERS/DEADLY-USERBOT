...
from userbot import HEROKU_APP, UPSTREAM_REPO_URL, deadlyub
...
async def upstream(event):
    ...
    if changelog == "" and not force_update:
        await event.edit(
            "\n`DEADLYUSERBOT is`  **up-to-date**  `with`  "
    ...
    if conf == "" and not force_update:
        ...
        await event.delete()
        return await event.respond(f"do `{cmdhd}update deploy` to update the Deadly")
...
async def upstream(event):
    ...
    off_repo = "https://github.com/deadly-fighters/nekopack"
    os.chdir("/app")
    try:
        txt = "`Oops.. Updater cannot continue due to "
        txt += "some problems occured`\n\n**LOGTRACE:**\n"
