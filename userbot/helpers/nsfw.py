...
async def importent(event):
    deadly = ["-1001199597035", "-1001459701099", "-1001436155389", "-1001321431101"]
    if str(event.chat_id) in deadly:
...
def nsfw(deadlyagory):
    deadlyagory.sort(key=str.casefold)
    horny = "**Catagory :** "
    for i in deadlyagory:
        horny += f" `{i.lower()}` ||"
    return horny
