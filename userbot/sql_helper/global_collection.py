...
class Deadly_GlobalCollection(BASE):
    __tablename__ = "deadly_globalcollection"
    ...
    def __repr__(self):
        return "<Deadly Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, Deadly_GlobalCollection)
            and self.keywoard == other.keywoard
            and self.contents == other.contents
        )


Deadly_GlobalCollection.__table__.create(checkfirst=True)

DEADLY_GLOBALCOLLECTION = threading.RLock()
...
def add_to_collectionlist(keywoard, contents):
    with DEADLY_GLOBALCOLLECTION:
        keyword_items = Deadly_GlobalCollection(keywoard, tuple(contents))
...
def rm_from_collectionlist(keywoard, contents):
    with DEADLY_GLOBALCOLLECTION:
        keyword_items = SESSION.query(Deadly_GlobalCollection).get(
...
def del_keyword_collectionlist(keywoard):
    with DEADLY_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(Deadly_GlobalCollection.keywoard)
            .filter(Deadly_GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(Deadly_GlobalCollection.keywoard).distinct().all()
...
def num_collectionlist():
    try:
        return SESSION.query(Deadly_GlobalCollection).count()
    finally:
        SESSION.close()
...
def num_collectionlist_items():
    try:
        return SESSION.query(
            func.count(distinct(Deadly_GlobalCollection.keywoard))
        ).scalar()
    finally:
        SESSION.close()
...
def __load_item_collectionlists():
    try:
        ...
        all_groups = SESSION.query(Deadly_GlobalCollection).all()
        for x in all_groups:
