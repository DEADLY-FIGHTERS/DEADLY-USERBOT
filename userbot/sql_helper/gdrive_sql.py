...
class Gdrive(BASE):
    __tablename__ = "deadlygdrive"
    deadly = Column(String(50), primary_key=True)

    def __init__(self, deadly):
        self.deadly = deadly
...
def is_folder(folder_id):
    try:
        return SESSION.query(Gdrive).filter(Gdrive.deadly == str(folder_id))
...
def rmparent_id(folder_id):
    note = SESSION.query(Gdrive).filter(Gdrive.deadly == folder_id)
    if note:
        note.delete()
        SESSION.commit()
