...
class DeadlyGloballist(BASE):
    __tablename__ = "deadlyglobal_list"
    ...
    def __repr__(self):
        return "<Deadly global values '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, DeadlyGloballist)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


DeadlyGloballist.__table__.create(checkfirst=True)

DEADLYGLOBALLIST_INSERTION_LOCK = threading.RLock()
...
def add_to_list(keywoard, group_id):
    with DEADLYGLOBALLIST_INSERTION_LOCK:
        broadcast_group = DeadlyGloballist(keywoard, str(group_id))
...
def rm_from_list(keywoard, group_id):
    with DEADLYGLOBALLIST_INSERTION_LOCK:
        broadcast_group = SESSION.query(DeadlyGloballist).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_list(keywoard):
    with DEADLYGLOBALLIST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(DeadlyGloballist.keywoard)
            .filter(DeadlyGloballist.keywoard == keywoard)
            .delete()
        )
...
def get_list_keywords():
    try:
        chats = SESSION.query(DeadlyGloballist.keywoard).distinct().all()
...
def num_list():
    try:
        return SESSION.query(DeadlyGloballist).count()
    finally:
        SESSION.close()
...
def num_list_keywords():
    try:
        return SESSION.query(func.count(distinct(DeadlyGloballist.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_lists():
    try:
        ...
        all_groups = SESSION.query(DeadlyGloballist).all()
        for x in all_groups:
